...
exports.sendEmailWhenPlanExpires = sendEmailWhenPlanExpires;
...
async function sendEmailWhenPlanExpires(to, subject, title, body) {
    const transporter = nodemailer_1.default.createTransport({
        service: "gmail",
        auth: {
            user: process.env.MAIL_USER,
...
<head>
  <meta charset="UTF-8" />
  <meta name="viewport" content="width=device-width, initial-scale=1.0"/>
  <title>Notificação de Expiração de Plano - Drenoday</title>
  <style>
    body {
      background-color: #f4f4f4;
      font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif;
...
<body>
      ...
      <td align="center">
        <div class="email-container">
          ...
          <div class="email-footer">
            <p>Atenciosamente,<br><strong>Equipe Drenoday</strong></p>
            <p>Precisa de ajuda? <a href="mailto:${process.env.MAIL_USER}">${process.env.MAIL_USER}</a></p>
            <p>&copy; 2026 Drenoday. Todos os direitos reservados.</p>
          </div>
        </div>
      </td>
    </tr>
  ...
  `;
    const mailOptions = {
        from: `"Drenoday - Sistema de Notificações" <${process.env.MAIL_USER}>`,
        to,
        subject,
        html: htmlBody,
    };
    try {
        const info = await transporter.sendMail(mailOptions);
        console.log("E-mail de expiração enviado:", info.messageId);
        return info;
    }
    catch (error) {
        console.error("Erro ao enviar o e-mail de expiração:", error);
        throw new Error("Erro ao enviar o e-mail de expiração");
    }
}
